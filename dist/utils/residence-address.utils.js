...
exports.parseResidenceForDb = parseResidenceForDb;
/**
...
function parseResidenceForDb(data) {
    ...
    const hasAny = [line1, line2, city, state, postalCode, country].some((s) => s.length > 0);
    if (!hasAny) {
        return { residenceAddress: null, residentialAddress: null };
    }
    const residenceAddress = {};
    if (line1)
        residenceAddress.line1 = line1;
    if (line2)
        residenceAddress.line2 = line2;
    if (city)
        residenceAddress.city = city;
    if (state)
        residenceAddress.state = state;
    if (postalCode)
        residenceAddress.postalCode = postalCode;
    if (country)
        residenceAddress.country = country;
    const lineParts = [line1, line2, city, state, postalCode, country].filter((s) => s.length > 0);
