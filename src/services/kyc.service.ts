...
function assertCorporateScalars(cp: {
  ...
  if (needsRegulatory) {
    if (!cp.regulatoryLicenseIssue)
      throw new Error(
        "Regulatory license issue date is required when a regulatory license number is provided",
      );
    if (!cp.regulatoryLicenseExpiry)
      throw new Error(
        "Regulatory license expiry is required when a regulatory license number is provided",
      );
  }
}
...
function assertCorporateKeyPersonnelAndShareholders(cp: {
  ...
  const kpOk = kp.some((row: unknown) => {
    if (!row || typeof row !== "object") return false;
    const o = row as Record<string, unknown>;
    const name = trimStr(String(o.fullName ?? ""));
    const id = trimStr(
      String(o.passportOrNationalId ?? o.passportOrNationalID ?? ""),
    );
    return Boolean(name && id);
  });
  if (!kpOk) {
    throw new Error(
      "Each key person must include full name and passport or national ID number",
    );
  }
  ...
  for (const row of sh) {
    ...
    if (kind === "CORPORATE") {
      ...
      if (!en || !rn || !addr) {
        throw new Error(
          "Corporate shareholders must include entity name, registration number, and registered address",
        );
      }
    } else {
      const fn = trimStr(String(o.fullName ?? ""));
      const idn = trimStr(
        String(o.idDocumentNumber ?? o.passportOrNationalId ?? ""),
      );
      if (!fn || !idn) {
        throw new Error(
          "Individual shareholders must include full name and ID document number",
        );
      }
    }
  }
...
function assertCorporateDocumentsUploaded(
  docs: { documentType: string }[],
  cp: { regulatoryLicenseNumber: string | null },
): void {
  const uploaded = new Set(docs.map((d) => d.documentType));
  for (const dt of CORPORATE_DOCUMENT_TYPES_REQUIRED) {
    if (!uploaded.has(dt)) {
      throw new Error(
        `Missing required document: ${dt.replace(/_/g, " ").toLowerCase()}`,
      );
    }
  }
  if (trimStr(cp.regulatoryLicenseNumber) && !uploaded.has("REGULATORY_LICENSE")) {
    throw new Error(
      "Regulatory license document upload is required when a regulatory license number is provided",
    );
  }
}
